#!/bin/sh
# Task 032 network source-boundary gate: native networking exists, but ONLY
...
# descriptor or readiness syscall. The connection I/O layer in
# src/connection_io.c owns bounded receive/send buffer interaction and
# read/write readiness handling through the existing recv/send primitives
# with no direct socket call, no protocol handling, and no heap allocation.
# The connection/session layer in src/connection_session.c coordinates the
# connection ownership model with the connection I/O lifecycle with no direct
...
# allocation. The admission coordinator in src/connection_admission.c only
# composes the existing accept, connection ownership, and manager APIs with
# fixed caller storage; it performs no direct socket call or heap allocation.
...
# The connection policy in src/connection_policy.c consumes one dispatch
# result, coordinates interest synchronization and admission release through
# existing owners, and performs no direct socket, readiness, descriptor, or
# heap operation. Interest updates flow through the connection-reactor and
# reactor APIs so poller, reactor, and connection metadata remain ordered.
...
# dispatch remains separate from listener admission; Task 032 lifecycle
# policy updates interests only through the existing owner chain. Later tasks
...
# Task 016/017/018/019/020/022/025/026/027/028/029/030/031/032 contracts).
...
HEAP_HITS=$(grep -nE "$NOHEAP_IN_ACCEPT" "$NATIVE_DIR/src/accepted.c" \
  ...
  "$NATIVE_DIR/src/connection_policy.c") || true
if [ -n "$HEAP_HITS" ]; then
  echo "FAIL: heap-allocation call in accepted/recv/send/connection/registry/connection_reactor/runtime/event_loop/connection_io/connection_session/connection_runtime/connection_manager/connection_admission/listener_admission/connection_dispatch/connection_policy production layer (see match above)" >&2
  echo "$HEAP_HITS" >&2
  FAIL=1
fi
