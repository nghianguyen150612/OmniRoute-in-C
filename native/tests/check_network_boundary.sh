#!/bin/sh
# Task 015 network source-boundary gate: native networking exists, but ONLY
# in the deliberate networking layer (src/listener.c owns socket lifecycle,
# src/poller.c owns the readiness wait). Every other production module
# (arena, bytebuf, meminfo, main, all other headers and sources) must stay
# socket-free, and the deferred layers — accept path, event loop, socket
# payload I/O, DNS/client, threads, TLS — stay banned from ALL production
# sources, including the networking layer itself. Later tasks extend this
# gate explicitly; they never loosen it silently.
#
...
# as the Task 011-014 gate). Tests under tests/ are intentionally NOT
...
# 2. Readiness-wait token: allowed ONLY in src/poller.c. poll() is
...
# 3. Deferred layers: banned in every production source, including
# src/listener.c and src/poller.c. Task 015 owns readiness only — no
# accept path, no event loop (epoll/select/io_uring/kqueue), no payload
# I/O (send/recv/read/write/shutdown), no DNS/client resolution, no
# threads, no TLS.
BANNED_EVERYWHERE='\<(accept\w*|connect|epoll_\w*|kqueue|kevent|io_uring|select|sendmsg|recvmsg|sendto|recvfrom|shutdown|send|recv|read|write|getaddrinfo|getnameinfo|pthread_\w*|SSL_\w*|TLS_\w*|mbedtls_\w*)\s*\('
...
echo "OK: networking confined to src/listener.c + src/poller.c; no accept/loop/IO/thread/TLS calls"
